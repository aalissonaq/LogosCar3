<?php
    ...
    if($_POST!=NULL){
        ...
        $iniciokm=@$_POST['iniciokm'];
        $fimkm=@$_POST['fimkm'];
        $fotoinicio=@$_POST['picinicio64'];
        $fotofim=@$_POST['picfim64'];
        $kmrodado = $fimkm-$iniciokm;
        //salvar tudo
        $query = $bd->prepare('INSERT INTO tb_rodagem(id_user,id_lotacao,id_veiculo,data_lancamento,data_fim,km_inicio,km_fim,total_rodado,foto_painel_ini, foto_painel_fim) VALUES (:id_user,:id_lotacao,:id_veiculo,:data_lancamento,:data_fim,:km_inicio,:km_fim,:total_rodado,:foto_painel_ini,:foto_painel_fim)');
        $query->bindParam(':id_user',$user);
        $query->bindParam(':id_lotacao',$lotacao);
        $query->bindParam(':id_veiculo',$veiculo);
        $query->bindParam(':data_lancamento',$datainicio);
        $query->bindParam(':data_fim',$datafim);
        $query->bindParam(':km_inicio',$iniciokm);
        $query->bindParam(':km_fim',$fimkm);
        $query->bindParam(':total_rodado',$kmrodado);
        $query->bindParam(':foto_painel_ini',$fotoinicio);
        $query->bindParam(':foto_painel_fim',$fotofim);
        //inserir no BD
        $query->execute();
        ...
        $acao = $user.' registrou nova rodagem via '.$modelo;
        $save->bindParam(':acao',$acao);
        $save->execute();

        $msg_titulo = '<h3>Tudo certo, '.$nome_curto[0].'!</h3>';
        $msg_corpo = 'As informações que você repassou estão salvas e você poderá consultá-las no menu Histórico.';
    } else{
