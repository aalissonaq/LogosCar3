<?php
    ...
    if($_POST!=NULL){
        //variaveis
        $montadora = $_POST['selectCar'];
        $modelo = $_POST['selectModel'];
        $anofab = $_POST['ano_fab'];
        $anomod = $_POST['ano_mod'];
        $placa = $_POST['placa'];
        $data_recebimento = $_POST['data_recebimento'];
        $condutor = $_POST['selectDriver'];
        if($condutor==0){
            $condutor=NULL;
        }
        $cadastrante = $user;
        ...
        //salvar tudo
        $query = $bd->prepare('INSERT INTO tb_veiculo(id_condutor,id_lotacao,montadora,modelo,ano_fab, modelo_fab, placa, resp_criacao, data_recebimento) VALUES (:id_condutor,:id_lotacao,:montadora,:modelo,:ano_fab,:modelo_fab,:placa,:resp_criacao,:data_recebimento)');
        $query->bindParam(':id_condutor',$condutor);
        $query->bindParam(':id_lotacao',$lotacao);
        $query->bindParam(':montadora',$montadora);
        $query->bindParam(':modelo',$modelo);
        $query->bindParam(':ano_fab',$anofab);
        $query->bindParam(':modelo_fab',$anomod);
        $query->bindParam(':placa',$placa);
        $query->bindParam(':resp_criacao',$user);
        ...
        $save = $bd->prepare('INSERT INTO tb_log (id_user,id_lotacao,ip,dispositivo,data_login,acao) VALUES (:user,:lotacao,:ip,:dispositivo,NOW(),:acao)');
        $save->bindParam(':user',$user);
        $save->bindParam(':lotacao',$lotacao);
        $save->bindParam(':dispositivo',$modelo);
        $save->bindParam(':ip',$meuip['ip']);
        $acao = $res->id_user.' cadastrou um '. $montadora .' '.$modelo.' '. $anofab.'/'. $anomod .' de placa '.$placa;
        $save->bindParam(':acao',$acao);
        $save->execute();

        $msg_titulo = '<h3>Tudo certo,'.$nome_curto[0].'!</h3>';
        if($condutor==NULL){
            $msg_corpo = 'O '. $montadora .' '.$modelo.' de placa '.$placa.' foi adicionado com sucesso.';
        } else{
            $query = $bd->prepare('SELECT nome_user FROM tb_users WHERE (id_user=:id_user)');
            $query->bindParam(':id_user',$condutor);
            $query->execute();
            $cndt = $query->fetchAll(PDO::FETCH_OBJ);
            $nome_condutor = explode(' ', $cndt->nome_user);
            $tamanho_nome = count($nome_condutor);
            $msg_corpo = 'O '. $montadora .' '.$modelo.' de placa '.$placa.' foi adicionado com sucesso sob a condução de'.$nome_condutor[0].' '.$nome_condutor[$tamanho_nome-1];
        }
        
    } else{
        $msg_titulo = '<h3>Ops! Ocorreu uma falha...</h3>';
        $msg_corpo = 'Houve algum problema e não foi possível executar o procedimento. Po favor tente novamente.';
    }
    header("refresh:10;url=veiculos.php");
?>
<!DOCTYPE html>
<html>
<head>
